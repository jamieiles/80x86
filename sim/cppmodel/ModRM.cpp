...
uint16_t ModRMDecoder::mod00()
{
    switch (modrm & 0x7) {
        case 0x0:
            return registers->get(BX) + registers->get(SI);
        case 0x1:
            return registers->get(BX) + registers->get(DI);
        case 0x2:
            return registers->get(BP) + registers->get(SI);
        case 0x3:
            return registers->get(BP) + registers->get(DI);
        case 0x4:
            return registers->get(SI);
        case 0x5:
            return registers->get(DI);
        case 0x6:
            return static_cast<uint16_t>(get_byte()) |
                   (static_cast<uint16_t>(get_byte()) << 8);
        case 0x7:
            return registers->get(BX);
    }
    __builtin_unreachable();
}
...
uint16_t ModRMDecoder::mod01()
{
    ...
    switch (modrm & 0x7) {
        case 0x0:
            return registers->get(BX) + registers->get(SI) + displacement;
        case 0x1:
            return registers->get(BX) + registers->get(DI) + displacement;
        case 0x2:
            return registers->get(BP) + registers->get(SI) + displacement;
        case 0x3:
            return registers->get(BP) + registers->get(DI) + displacement;
        case 0x4:
            return registers->get(SI) + displacement;
        case 0x5:
            return registers->get(DI) + displacement;
        case 0x6:
            return registers->get(BP) + displacement;
        case 0x7:
            return registers->get(BX) + displacement;
    }
    __builtin_unreachable();
}
