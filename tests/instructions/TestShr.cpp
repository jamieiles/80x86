...
INSTANTIATE_TEST_CASE_P(Shr1, ShiftReg8Test,
    ::testing::Values(
        // shr al, 1
        Shift8Params({ 0xd0, 0xe8 }, shr8_shift1_tests)
    ));

INSTANTIATE_TEST_CASE_P(Shr1, ShiftMem8Test,
    ::testing::Values(
        // shr byte [bx], 1
        Shift8Params({ 0xd0, 0x2f }, shr8_shift1_tests)
    ));

INSTANTIATE_TEST_CASE_P(Shr1, ShiftReg16Test,
    ::testing::Values(
        // shr ax, 1
        Shift16Params({ 0xd1, 0xe8 }, shr16_shift1_tests)
    ));

INSTANTIATE_TEST_CASE_P(Shr1, ShiftMem16Test,
    ::testing::Values(
        // shr word [bx], 1
...
INSTANTIATE_TEST_CASE_P(ShrN, ShiftMem8Test,
    ::testing::Values(
        // shr byte [bx], N
        Shift8Params({ 0xd2, 0x2f }, shr8_shiftN_tests)
    ));

INSTANTIATE_TEST_CASE_P(ShrN, ShiftReg16Test,
    ::testing::Values(
        // shr ax, N
        Shift16Params({ 0xd3, 0xe8 }, shr16_shiftN_tests)
    ));

INSTANTIATE_TEST_CASE_P(ShrN, ShiftMem16Test,
    ::testing::Values(
        // shr word [bx], N
        Shift16Params({ 0xd3, 0x2f }, shr16_shiftN_tests)
    ));

INSTANTIATE_TEST_CASE_P(ShrN, ShiftRegImm8Test,
    ::testing::Values(
        // shr ax, imm8
        Shift8Params({ 0xc0, 0xe8}, shr8_shiftN_tests)
    ));

INSTANTIATE_TEST_CASE_P(ShrN, ShiftMemImm8Test,
    ::testing::Values(
        // shr word [bx], imm8
        Shift8Params({ 0xc0, 0x2f}, shr8_shiftN_tests)
    ));

INSTANTIATE_TEST_CASE_P(ShrN, ShiftRegImm16Test,
    ::testing::Values(
        // shr ax, imm16
        Shift16Params({ 0xc1, 0xe8}, shr16_shiftN_tests)
    ));

INSTANTIATE_TEST_CASE_P(ShrN, ShiftMemImm16Test,
    ::testing::Values(
        // shr word [bx], imm16
        Shift16Params({ 0xc1, 0x2f}, shr16_shiftN_tests)
    ));

INSTANTIATE_TEST_CASE_P(ShrCL, ShiftCLTest,
    ::testing::Values(
        ShiftCLTestParams{4, 0, { 0xd2, 0xe9 }}
    ));
