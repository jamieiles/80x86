...
static const std::vector<struct ShiftTest<uint16_t>> sar16_shift1_tests = {
    ...
    { 1, 2, 1, 0, 0 },
    { 1, 0, 0, CF, PF | ZF },
    { 1, 0x8000, 0xc000, CF, PF | SF },
    { 1, 0x8000, 0xc000, 0, PF | SF },
};

INSTANTIATE_TEST_CASE_P(Sar1, ShiftReg8Test,
    ::testing::Values(
        // sar al, 1
        Shift8Params({ 0xd0, 0xf8 }, sar8_shift1_tests)
    ));

INSTANTIATE_TEST_CASE_P(Sar1, ShiftMem8Test,
    ::testing::Values(
        // sar byte [bx], 1
        Shift8Params({ 0xd0, 0x3f }, sar8_shift1_tests)
    ));

INSTANTIATE_TEST_CASE_P(Sar1, ShiftReg16Test,
    ::testing::Values(
        // sar ax, 1
        Shift16Params({ 0xd1, 0xf8 }, sar16_shift1_tests)
    ));

INSTANTIATE_TEST_CASE_P(Sar1, ShiftMem16Test,
    ::testing::Values(
        // sar word [bx], 1
...
static const std::vector<struct ShiftTest<uint16_t>> sar16_shiftN_tests = {
    ...
    { 1, 0x8000, 0xc000, 0, PF | SF },
    { 16, 0, 0, 0, PF | ZF },
    { 15, 0x8000, 0xffff, 0, PF | SF },
    { 16, 0x8000, 0xffff, 0, PF | SF | CF }
};

INSTANTIATE_TEST_CASE_P(SarN, ShiftReg8Test,
    ::testing::Values(
        // sar al, N
        Shift8Params({ 0xd2, 0xf8 }, sar8_shiftN_tests)
    ));

INSTANTIATE_TEST_CASE_P(SarN, ShiftMem8Test,
    ::testing::Values(
        // sar byte [bx], N
        Shift8Params({ 0xd2, 0x3f }, sar8_shiftN_tests)
    ));

INSTANTIATE_TEST_CASE_P(SarN, ShiftReg16Test,
    ::testing::Values(
        // sar ax, N
        Shift16Params({ 0xd3, 0xf8 }, sar16_shiftN_tests)
    ));

INSTANTIATE_TEST_CASE_P(SarN, ShiftMem16Test,
    ::testing::Values(
        // sar word [bx], N
